...
void omni_wheel_inverse_kinematics(Velocity_Input input, float motor_speeds[4]) {
    ...
    // 全向轮运动学逆解公式[4,6](@ref)
    // 考虑轮子安装角度为45°的情况
    motor_speeds[0] = ( input.vx * cos45 - input.vy * cos45 - input.omega * R) / chassis_params.wheel_radius;  // 前左轮
    motor_speeds[1] = (-input.vx * cos45 - input.vy * cos45 - input.omega * R) / chassis_params.wheel_radius;  // 前右轮
    motor_speeds[2] = ( input.vx * cos45 + input.vy * cos45 + input.omega * R) / chassis_params.wheel_radius;  // 后左轮
    motor_speeds[3] = (-input.vx * cos45 + input.vy * cos45 + input.omega * R) / chassis_params.wheel_radius;  // 后右轮
...
/**
 * @brief 通用运动学逆解（适用于任意安装角度）
 * @param input 速度输入
 * @param motor_speeds 输出速度
 * @param angles 四个轮子的安装角度（弧度）
 * 
 * 通用公式：V_i = [cosθ_i, sinθ_i] · [vx, vy] + ω × R_i
 */
void general_omni_kinematics(Velocity_Input input, float motor_speeds[4], float angles[4]) {
    float half_len = chassis_params.length / 2.0f;
    float half_wid = chassis_params.width / 2.0f;
    
    // 四个轮子的位置向量
    float positions[4][2] = {
        {-half_len,  half_wid},  // 前左
        { half_len,  half_wid},  // 前右
        {-half_len, -half_wid},  // 后左
        { half_len, -half_wid}   // 后右
    };
    
    for (int i = 0; i < 4; i++) {
        // 计算旋转分量：ω × r
        float rot_component = input.omega * 
                            (positions[i][0] * sinf(angles[i]) - 
                             positions[i][1] * cosf(angles[i]));
        
        // 计算平移分量：[cosθ, sinθ] · [vx, vy]
        float trans_component = input.vx * cosf(angles[i]) + 
                              input.vy * sinf(angles[i]);
        
        motor_speeds[i] = (trans_component + rot_component) / chassis_params.wheel_radius;
    }
}